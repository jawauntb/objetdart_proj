...
const trailRoute = readText("apps/native/app/trail.tsx");
const guideRoute = readText("apps/native/app/guide.tsx");
const trailView = readText("apps/native/src/trail/TrailView.tsx");
const historyEventView = readText("apps/native/src/trail/HistoryEventView.tsx");
const branchView = readText("apps/native/src/trail/BranchView.tsx");
const trailModel = readText("apps/native/src/trail/model.ts");
const conceptReveal = readText("apps/native/src/guide/ConceptReveal.tsx");
const conceptRevealLaw = readText("apps/native/src/guide/conceptAccess.ts");
const localFirstValidation = readText("docs/native/local-first-validation.md");
...
assert.match(proofRoute, /\/trail\?scene=/, "proof scenes must open the dedicated sought trail route");
assert.match(proofRoute, /\/guide\?scene=/, "proof scenes must open the dedicated sought guide route");
...
assert.match(trailRoute, /<TrailView/, "the dedicated trail route must render the natural-history projection");
assert.match(guideRoute, /<ConceptReveal/, "the dedicated guide route must render canonical concept reveals");
assert.match(trailView, /cause and consequence, not a score/i, "the trail must refuse achievement framing");
assert.match(historyEventView, /return to/, "history events must expose scene return anchors");
assert.match(branchView, /retirementTransition/, "branch retirement UI must use the reversible transition law");
assert.doesNotMatch(branchView, /purge/i, "branch cards must not expose irreversible purge");
assert.match(trailModel, /collision/, "the natural-history projection must represent collisions");
assert.match(conceptRevealLaw, /direct-seeking/, "concept reveals must allow explicit seeking");
assert.match(conceptRevealLaw, /accessibility/, "concept reveals must allow assistive requests");
assert.match(conceptReveal, /conceptRevealFor/, "the visual reveal must use the same access law as tests");
assert.match(localFirstValidation, /pending/i, "local-first validation must state that human evidence is pending");
assert.match(localFirstValidation, /export/i, "local-first validation must keep permanent removal export-first");
