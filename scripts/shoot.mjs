// Drive the /tourbillon page through camera presets and save PNGs for review.
...
const url = `${base}/tourbillon?spin=0&view=iso&shot=1`;
console.log("navigating", url);
await page.goto(url, { waitUntil: "networkidle", timeout: 60000 });

// wait for the tourbillon hook + a few rendered frames
await page.waitForFunction(() => window.__tourbillon && window.__tourbillon.ready === true, { timeout: 30000 });
await page.waitForTimeout(1200);

for (const v of VIEWS) {
  await page.evaluate((name) => window.__tourbillon.setView(name), v);
