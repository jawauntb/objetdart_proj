...
| `/light` | The scale lattice now blooms under the active lens, and "listen" teaches chord shapes with ghost hands; touches and lessons leave tape marks, though synthesis still runs on the dedicated light voice. | 1. Route tones through the shared field audio where possible. 2. Add a lingering afterimage trail on kept marks. 3. Let shake/replay feel more like the water template. | Medium | Batch B (partial) |
...
| `/time` | Now the relativity manifold, not the old chronograph (the audit's original row described `TimeCabinet`, deleted since). One, two and three-finger layers and the vessel all answer in sight, sound and touch, and the clock's flow rides the shared turbulence axis. | 1. The two-finger twist-lens (worldline → felt duration → bare metric) could earn a third level of its own rather than crossfading three. 2. Tilt-gravity has no touch equivalent for the lateral lean. 3. No scale address — a deliberate exemption, like the other non-spatial instrument rooms. | Low | Done |
