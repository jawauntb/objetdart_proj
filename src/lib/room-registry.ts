...
/**
 * The disciplines a room can teach with — the vocabulary of the nav's
 * discovery chips, nothing more. Not a taxonomy of the axis (that is
 * `SCALE_BANDS`) and not a cluster (that is `SITE_ROUTES`): a stranger
 * tapping `music` in the dropdown should find every room whose material
 * speaks it. Fixed list; a room picks 1–3, honestly, from its material.
 * `scripts/test-nav-groups.mjs` fails an entry with none, more than three,
 * or a tag no room uses (a chip that filters to nothing is a dead control).
 */
export const NAV_DISCIPLINES = [
  "physics",
  "math",
  "music",
  "biology",
...
export type NavDiscipline = (typeof NAV_DISCIPLINES)[number];
...
export type RoomEntry = {
  ...
  href: string;
  kind: RoomKind;
  /** what the room teaches with, for nav discovery — from NAV_DISCIPLINES */
  disciplines?: readonly NavDiscipline[];
...
export const ROOM_REGISTRY: RoomEntry[] = [
  ...
  },
  {
    ...
    href: "/manifold",
    kind: "room",
    disciplines: ["physics", "metaphysics"],
